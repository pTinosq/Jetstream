<script lang="ts">
  ...
  // Open the add-flight form by default only when there's nothing to show yet.
  // Captured once so saving the first flight doesn't snap the form shut.
  const addOpen = untrack(() => data.flights.length === 0);
...
<div class="enter mx-auto max-w-5xl px-4 py-10 sm:px-6">
  <div class="mb-10">
    <CollapsibleSection
      title="Add a flight"
      subtitle="Search a real flight, paste a booking, or enter it by hand"
      open={addOpen}
    >
      ...
      <div class="mb-6 rounded-panel border border-line bg-white/45 p-4">
        ...
        <div class="mt-3 flex flex-wrap items-center gap-3">
          ...
          {:else if aiMatched === false}
            <span
              class="rounded-full bg-amber-500/12 px-2.5 py-1 text-xs font-medium text-amber-800"
            >
        ...
        <div class="rounded-panel border border-line bg-white/45 p-4">
          <div class="flex flex-wrap items-end gap-3">
            ...
            <p class="max-w-xs text-xs leading-relaxed text-ink-mute">
              Pick From, To and a date to look up flights. Works best for recent flights (roughly
              the last few weeks).
            </p>
          </div>
